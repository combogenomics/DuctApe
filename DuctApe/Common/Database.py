...
class Biolog(DBBase):
    ...
    def isMulti(self, plate_id, well_id):
        ...
        '''
        with self.connection as conn:
            cursor=conn.execute('select concentration from biolog where plate_id=? and well_id=?;',
                                [plate_id,well_id,])
    ...
    def getCategs(self):
        with self.connection as conn:
            cursor=conn.execute('select distinct category from biolog order by plate_id;')
    ...
    def addExps(self, explist):
        ...
        with self.connection as conn:
            ...
            blist = ['''(%s,%s,%s,%s,%s,%s,%s)'''
                     %(b.plate_id,b.well_id,b.org_id,b.replica,
                              b.active,b.prediction, b.zero)
                          for b in explist]
            for bs in get_span(blist, span=50):
                insert = query + ','.join(bs)+';'
                conn.execute(insert)
    
    def delExps(self, explist):
        '''
        Input: a series of BiologExp objects
        '''
        with self.connection as conn:
    ...
    def setZeros(self, explist):
        '''
        Input: a series of BiologExp objects
        '''
        with self.connection as conn:
            for b in explist:
                conn.execute('''update biolog_exp set  
                            where plate_id=? and well_id=? and org_id=?
                            and replica=?;''',
                            [b.plate_id,b.well_id,b.org_id,b.replica,])
    
    def isZero(self, plate_id, well_id, org_id, replica):
        with self.connection as conn:
    ...
    def isActive(self, plate_id, well_id, org_id, replica):
        with self.connection as conn:
            cursor=conn.execute('''select active from biolog_exp 
                            where plate_id=? and well_id=? and org_id=?
                            and replica=?;''',
                            [plate_id,well_id,org_id,replica,])
        return bool(cursor.fetchall()[0][0])
    
    def getActiveByPlate(self, plate_id, replica):
        with self.connection as conn:
            cursor=conn.execute('''select * from biolog_exp  
                        where plate_id=? and replica=?
                        and active=true;''',
                        [plate_id,replica,])
